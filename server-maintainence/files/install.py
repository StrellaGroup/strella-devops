...
def install_bench(args):
	...
	extra_vars.update(get_passwords(args, args.run_travis or args.without_bench_setup))
	if args.production:
...
def get_passwords(args, ignore_prompt=False):
	if not ignore_prompt:
		if not args.mysql_root_password:
			mysql_root_password = ''
		else:
...
def parse_commandline_args():
	...
	parser.add_argument('--mysql_root_password', dest='mysql_root_password', help='MYSQL Root Password')
	
	parser.add_argument('--admin_password', dest='admin_password', help='Default Administrator Password')
